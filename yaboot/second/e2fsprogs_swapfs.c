...
void ext2fs_swap_ext_attr(char *to, char *from, int bufsize, int has_header)
{
	...
	while ((char *)from_entry < from_end && *(u32 *)from_entry) {
			    ...
			    int bufsize)
{
	...
	if (bufsize < (int) (sizeof(struct ext2_inode) + sizeof(u16)))
	...
	i = sizeof(struct ext2_inode) + t->i_extra_isize + sizeof(u32);
	if (bufsize < (int) i)
		return; /* no space for EA magic */

	eaf = (u32 *) (((char *) f) + sizeof(struct ext2_inode) +
					f->i_extra_isize);

	if (ext2fs_swab32(*eaf) != EXT2_EXT_ATTR_MAGIC)
		return; /* it seems no magic here */

	eat = (u32 *) (((char *) t) + sizeof(struct ext2_inode) +
					f->i_extra_isize);
