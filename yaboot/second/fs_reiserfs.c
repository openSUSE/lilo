...
static int reiserfs_read(struct boot_file_t *file, unsigned int size, void *buffer)
{
...
static __inline__ int is_power_of_two(unsigned long word)
{
	...
	while (transactions-- > 0) {
		...
		if (*journal_table != 0xffffffff) {
			...
			if (j_len >= JOURNAL_TRANS_HALF) {
				...
				if (!journal_read(commit_block, sizeof(commit), (char *)&commit))
					return 0;
...
static int journal_init(void)
{
	...
	while (1) {
		...
		if (strcmp(JOURNAL_DESC_MAGIC, desc.j_magic) != 0
		    || desc.j_trans_id != next_trans_id || desc.j_mount_id != header.j_mount_id)
			/* no more valid transactions */
			break;

		commit_block = (desc_block + le32_to_cpu(desc.j_len) + 1) & (block_count - 1);
		journal_read(commit_block, sizeof(commit), (char *)&commit);
		if (desc.j_trans_id != commit.j_trans_id || desc.j_len != commit.j_len)
			/* no more valid transactions */
			break;

		DEBUG_F("Found valid transaction %u/%u at %u.\n",
			le32_to_cpu(desc.j_trans_id), le32_to_cpu(desc.j_mount_id), desc_block);
				...
				*journal_table++ = desc.j_len;
				...
				for (; i < le32_to_cpu(desc.j_len); i++) {
					*journal_table++ = commit.j_realblock[i - JOURNAL_TRANS_HALF];
					...
					DEBUG_F("block %u is in journal %u.\n",
						le32_to_cpu(commit.j_realblock[i - JOURNAL_TRANS_HALF]), desc_block);
	...
	INFO->journal_transactions = next_trans_id - le32_to_cpu(header.j_last_flush_trans_id) - 1;
	return (errnum == 0);
}
...
static int reiserfs_read_super(void)
{
	...
	if (strcmp(REISER2FS_SUPER_MAGIC_STRING, super.s_magic) != 0 && strcmp(REISERFS_SUPER_MAGIC_STRING, super.s_magic) != 0) {
	...
	if (INFO->blocksize < FSYSREISER_MIN_BLOCKSIZE || INFO->blocksize > FSYSREISER_MAX_BLOCKSIZE) {
	...
	if (INFO->journal_block != 0) {
		if (!is_power_of_two(INFO->journal_block_count)) {
			prom_printf("ReiserFS: Unsupported journal size, " "not a power of 2: %u\n", INFO->journal_block_count);
			return 0;
		}

		journal_init();
		/* Read in super block again, maybe it is in the journal */
		block_read(superblock, 0, sizeof(struct reiserfs_super_block), (char *)&super);
	}

	/* Read in the root block */
	if (!block_read(le32_to_cpu(super.s_root_block), 0, INFO->blocksize, ROOT)) {
...
static char *read_tree_node(__u32 blockNr, __u16 depth)
{
	...
	DEBUG_F("FOUND: blk_level=%u, blk_nr_item=%u, blk_free_space=%u\n",
		blkh_level(BLOCKHEAD(cache)), blkh_nr_item(BLOCKHEAD(cache)), le16_to_cpu(BLOCKHEAD(cache)->blk_free_space));
...
static int next_key(void)
{
	...
	DEBUG_F("next_key:\n  old ih: key %u:%u:%u:%u version:%u\n",
		...
		le32_to_cpu(INFO->current_ih->ih_key.u.k_offset_v1.k_uniqueness), ih_version(INFO->current_ih));
...
static int search_stat(__u32 dir_id, __u32 objectid)
{
	...
	DEBUG_F("search_stat:\n  key %u:%u:0:0\n", le32_to_cpu(dir_id), le32_to_cpu(objectid));
	...
	while (depth > BLKH_LEVEL_LEAF) {
		...
		for (i = 0; i < nr_item; i++) {
			if (le32_to_cpu(key->k_dir_id) > le32_to_cpu(dir_id)
			    || (key->k_dir_id == dir_id && (le32_to_cpu(key->k_objectid) > le32_to_cpu(objectid)
	...
	for (i = 0; i < nr_item; i++) {
		if (ih->ih_key.k_dir_id == dir_id
		    ...
		    && ih->ih_key.u.k_offset_v1.k_offset == 0 && ih->ih_key.u.k_offset_v1.k_uniqueness == 0) {
...
static int reiserfs_open_file(char *dirname)
{
	...
	while (1) {
		...
		DEBUG_F("sd_mode=0%o sd_size=%Lu\n",
			sd_mode((struct stat_data *)INFO->current_item), sd_size(INFO->current_ih, INFO->current_item));
