...
static inline void outw(int port, unsigned short val)
{
	outb(port, val >> 8);
	outb(port + 1, val);
}

static long SwapBytes(long lv)
 ...
 */
static void unlockVideo(int slot)
{
...
static void setTextCLUT(int shift)
{
	int i;
...
static void loadFont(unsigned char *ISA_mem)
{
	int i, j;
 ...
 */
void writeAttr(unsigned char index, unsigned char data, unsigned char videoOn)
{
	unsigned char v;
...
static void setTextRegs(struct VgaRegs *svp)
{
	int i;
...
int vga_init(unsigned char *ISA_mem)
{
	int slot;
